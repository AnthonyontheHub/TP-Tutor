...
export default function ProgressSummary({ activeFilter, onFilterClick }: Props) {
  ...
  const statusItems: { status: MasteryStatus; label: string; emoji: string; color: string; glow: string }[] = [
    ...
    { status: 'practicing',  label: 'PRACTICING',  emoji: '🟡', color: '#a855f7', glow: 'rgba(168,85,247,0.6)'  },
    { status: 'confident',   label: 'CONFIDENT',   emoji: '🟢', color: '#22c55e', glow: 'rgba(34,197,94,0.6)'   },
    { status: 'mastered',    label: 'MASTERED',    emoji: '✦', color: '#ffbf00', glow: 'rgba(255,191,0,0.65)'  },
  ];
  return (
