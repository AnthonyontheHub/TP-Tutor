...
import { essentializerData, type EssentializerDrill } from '../../data/drills';
...
export const Essentializer = ({ onSessionEnd, onAskLina }) => {
  ...
  return (
    <div className="max-w-lg mx-auto min-h-[500px] flex flex-col relative font-sans">
      <div className="flex justify-between items-center mb-8 px-4">
        ...
        <motion.div key="content" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
          {currentChallenge ? (
            <>
              <div className="bg-black/60 backdrop-blur-2xl border border-white/10 p-6 rounded-[2rem] shadow-2xl relative overflow-hidden group">
                  ...
                  <p className="text-xl font-light leading-relaxed text-white/90 italic tracking-wide">"{currentChallenge.englishPrompt}"</p>
              </div>
              ...
              <div className="space-y-4">
                {!feedback ? (
                    <div className="grid gap-4">
                      {currentChallenge.options.map((opt, i) => (
                        <button 
                          ...
                          className="w-full p-4 text-left rounded-xl border border-white/5 bg-white/5 hover:border-rose-500/40 hover:bg-rose-500/10 transition-all group uppercase tracking-[0.15em] text-white/60 hover:text-white text-sm font-medium"
                        >
                          {opt}
                        </button>
                      ))}
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                      <div className={`p-6 rounded-[1.5rem] border ${feedback.score >= 70 ? 'border-rose-500/30 bg-rose-500/5' : 'border-white/10 bg-white/5'}`}>
                        <p className="text-sm text-white/80 leading-relaxed italic">"{feedback.feedback}"</p>
                      </div>
                      <button 
                      onClick={loadNextChallenge} 
                      className="w-full py-3 bg-rose-600 text-white text-sm font-black uppercase tracking-widest rounded-xl hover:bg-rose-500 transition-all flex items-center justify-center gap-3"
                    >
