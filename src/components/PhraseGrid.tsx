...
export default function PhraseGrid({ onAskLina, selectedWords, focusPhraseId, clearFocusPhrase }: Props) {
  ...
  const safeSavedPhrases = useMemo(() => savedPhrases || [], [savedPhrases]);
    ...
    <section className="phrase-grid">
              ...
              <div key={p.id} style={{ background: '#0a0a0a', padding: '18px', borderRadius: '12px', border: '1px solid #1e293b', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                ...
                <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #1e293b' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button type="button" onClick={() => { if(window.confirm('Delete this saved phrase?')) deletePhrase(p.id); }} className="btn-toggle" style={{ padding: '6px 12px', fontSize: '0.7rem', width: 'auto', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', borderRadius: '6px' }}>
                      DELETE
                    </button>
                  </div>
                </div>
              ...
              {Object.entries(groupedCommonPhrases).map(([cat, phrases]) => {
                ...
                return (
                  <div key={cat}>
                    ...
                    <div style={{ display: 'grid', gap: '12px', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
                      {(filtered || []).map((p) => (
                        <div key={p.tp} className="glass-panel" style={{ padding: '20px', background: 'rgba(5,5,5,0.4)', border: '1px solid #222', position: 'relative', overflow: 'hidden' }}>
      ...
      {editingId && (
        <div 
          ...
          role="button"
          tabIndex={0}
          onClick={() => { setEditingId(null); clearFocusPhrase?.(); }} 
          onKeyDown={(e) => { if (e.key === 'Escape' || e.key === 'Enter') { setEditingId(null); clearFocusPhrase?.(); } }}
        ...
        >
          <div onClick={e => e.stopPropagation()} style={{ background: '#0a0a0a', padding: '32px', borderRadius: '24px', width: '90%', maxWidth: '450px', border: '1px solid #222', boxShadow: '0 30px 60px rgba(0,0,0,0.8)' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button type="button" onClick={() => { updatePhraseNote(editingId, noteInput); setEditingId(null); clearFocusPhrase?.(); }} className="btn-review" style={{ margin: 0, flex: 2, height: '50px' }}>SAVE NOTE</button>
              <button type="button" onClick={() => { setEditingId(null); clearFocusPhrase?.(); }} className="btn-toggle" style={{ flex: 1, height: '50px', background: 'rgba(255,255,255,0.05)' }}>CANCEL</button>
            </div>
          </div>
        </div>
      )}
