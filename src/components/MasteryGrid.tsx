    ...
    <section className="mastery-grid" onClick={() => { if(selectedWords.length > 0) soundService.playBlip(220, 'sine', 0.05); setSelectedWords([]); }} style={{ paddingBottom: selectedWords.length > 1 ? '240px' : '20px' }}>
      ...
      {selectedWords.length > 0 && (
        <div style={{ position: 'fixed', bottom: '24px', left: '16px', right: '16px', background: '#111', border: '1px solid #3b82f6', borderRadius: '16px', padding: '16px', zIndex: 1000, boxShadow: '0 -10px 25px rgba(0,0,0,0.8)' }}>
          ...
          <div style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '12px', borderTop: selectedWords.length > 1 ? '1px solid #222' : 'none', paddingTop: selectedWords.length > 1 ? '10px' : '0', fontWeight: 'bold' }}>{selectedWords.join(' ')}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '8px' }}>
            <button onClick={() => { soundService.playBlip(880, 'sine', 0.1); onAskLina(`toki Lina! Is "${selectedWords.join(' ')}" a good sentence?`); setSelectedWords([]); }} style={{ padding: '12px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>ASK LINA</button>
            <button onClick={() => { soundService.playBlip(1046.50, 'sine', 0.1); savePhrase(selectedWords.join(' ')); setSelectedWords([]); }} style={{ padding: '12px', background: '#222', color: 'white', border: '1px solid #444', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.8rem' }}>SAVE 📌</button>
          </div>
        </div>
      )}
      {drawerId && (
