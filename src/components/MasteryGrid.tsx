    ...
    <section className="mastery-grid" onClick={() => setSelectedWords([])} style={{ paddingBottom: selectedWords.length > 1 ? '140px' : '20px' }}>
        ...
        {displayedVocab.map((word) => {
          ...
          return (
            <div key={word.id} onClick={(e) => { e.stopPropagation(); handleCardClick(word); }}
              style={{ transform: isSuperFocus ? 'scale(1.8) translateY(-15px)' : (isSelected ? 'scale(1.1)' : (selectedWords.length > 0 && !isSelected ? 'scale(0.85)' : 'scale(1)')), opacity: selectedWords.length > 0 && !isSelected ? 0.2 : 1, transition: 'all 0.3s ease', zIndex: isSelected ? 100 : 1, cursor: 'pointer', position: 'relative' }}
            >
      ...
      {selectedWords.length > 1 && (
        <div style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', background: '#111', border: '1px solid #3b82f6', borderRadius: '12px', padding: '15px', zIndex: 1000 }}>
          <div style={{ color: 'white', marginBottom: '10px' }}>{selectedWords.join(' ')}</div>
          <button onClick={() => { onAskLina(`Is "${selectedWords.join(' ')}" correct?`); setSelectedWords([]); }} style={{ width: '100%', padding: '10px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold' }}>ASK LINA</button>
        </div>
      )}
