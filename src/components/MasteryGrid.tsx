...
// Frequency Data Map (Keeping your source data for sorting)
...
export default function MasteryGrid({ onAskLina, isSandboxMode, activeFilter, selectedWords, setSelectedWords, sortMode }: Props) {
  ...
  const displayedVocab = [...vocabulary]
    ...
    .sort((a, b) => {
      if (sortMode === 'usage') {
        const freqA = WORD_FREQ[a.word] || 0;
        const freqB = WORD_FREQ[b.word] || 0;
        return freqB - freqA;
      }
  ...
  return (
    <section className="mastery-grid">
      {/* Framer Motion will now watch this div for children changes */}
        ...
        <AnimatePresence>
          {displayedVocab.map((word) => {
            ...
            return (
              <motion.div 
                key={word.id}
                layout // THIS ENABLES THE SORT ANIMATION
                initial={{ opacity: 0, scale: 0.9 }}
                animate={{ opacity: isDimmed ? 0.4 : 1, scale: isSelected ? 1.05 : 1 }}
                exit={{ opacity: 0, scale: 0.9 }}
                transition={{ 
                  type: "spring", 
                  stiffness: 300, 
                  damping: 30, 
                  layout: { duration: 0.4 } // Speed of the cards moving into place
                }}
              ...
              >
                ...
                {isSelected && (
                  <div style={{ position: 'absolute', top: '-8px', right: '-8px', background: '#3b82f6', color: 'white', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 'bold', border: '2px solid #fff', boxShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
                    {selectIndex + 1}
                  </div>
                )}
              </motion.div>
