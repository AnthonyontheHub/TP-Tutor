...
// Frequency Data Map (same as before)
...
export default function MasteryGrid({ onAskLina, isSandboxMode, activeFilter, selectedWords, setSelectedWords, sortMode }: Props) {
  ...
  // PERFORMANCE FIX: Only render a chunk of words at a time
  ...
  const sortedAndFilteredVocab = useMemo(() => {
    return [...vocabulary]
      ...
      .sort((a, b) => {
        if (sortMode === 'usage') {
          return (WORD_FREQ[b.word] || 0) - (WORD_FREQ[a.word] || 0);
        }
  ...
  }, [vocabulary, activeFilter, sortMode]);

  // The actual slice we display
    ...
    <section className="mastery-grid" style={{ padding: '0 16px' }} onClick={() => setSelectedWords([])}>
      ...
      {visibleCount < sortedAndFilteredVocab.length && (
        <div style={{ textAlign: 'center', padding: '40px 0' }}>
          <button 
            onClick={(e) => { e.stopPropagation(); setVisibleCount(prev => prev + 40); }}
            style={{ 
          ...
          >
            LOAD MORE WORDS ({sortedAndFilteredVocab.length - visibleCount} REMAINING)
          </button>
        </div>
      )}
