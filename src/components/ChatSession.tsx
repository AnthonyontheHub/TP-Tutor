...
export default function ChatSession({ onEndSession, isActive, pendingPrompt, clearPrompt }: Props) {
  ...
  async function sendToLina(txt: string) {
    ...
    setIsLoading(true);
    const userMsg = { id: crypto.randomUUID(), role: 'user', displayContent: txt };
    ...
    try {
      ...
      console.error(e); 
      setMessages(p => p.map(m => m.id === assistantId ? { ...m, displayContent: 'Sorry, I encountered an error. Please check your API key.' } : m));
    } finally { 
      setIsLoading(false); 
    }
  }
        ...
        {isActive && (
          <>
            ...
            <m.div 
              ...
              style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '92vh', zIndex: 2000, background: '#111', borderTopLeftRadius: '20px', borderTopRightRadius: '20px', display: 'flex', flexDirection: 'column', borderTop: '4px solid white' }}
            >
              <div style={{ width: '100%', padding: '16px 0', cursor: 'pointer', flexShrink: 0 }} onClick={onEndSession}>
                <div style={{ width: '48px', height: '6px', backgroundColor: '#666', borderRadius: '10px', margin: '0 auto' }} />
              </div>
              
              <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
                 {!localStorage.getItem('TP_GEMINI_KEY') && <div style={{ color: '#ff6b6b', textAlign: 'center', marginBottom: '20px', fontWeight: 'bold' }}>API Key required in Settings.</div>}
                 ...
                 {messages.map((msg) => (
                   <div key={msg.id} style={{ marginBottom: '20px', textAlign: msg.role === 'user' ? 'right' : 'left' }}>
                     ...
                     <div style={{ background: msg.role === 'assistant' ? '#1a1a1a' : '#3b82f6', padding: '12px', borderRadius: '8px', color: 'white', display: 'inline-block', textAlign: 'left', maxWidth: '85%', border: msg.role === 'assistant' ? '1px solid #333' : 'none' }}>
                       {msg.displayContent}
                     </div>
                     
                     {msg.proposedChanges && !msg.changesApplied && msg.role === 'assistant' && (
                       <div style={{ marginTop: '10px', background: '#000', padding: '12px', borderRadius: '8px', textAlign: 'left', border: '2px solid #10b981' }}>
                          <div style={{ fontSize: '0.7rem', color: '#10b981', marginBottom: '8px', fontWeight: 'bold' }}>PROPOSED UPDATES</div>
                          ...
                          <button onClick={() => handleApplyChanges(msg.id)} className="btn-review" style={{ margin: '10px 0 0 0', padding: '10px', fontSize: '0.8rem' }}>
                            APPLY CHANGES
                          </button>
                       </div>
                     )}
              ...
              <div style={{ padding: '16px', background: '#0a0a0a', borderTop: '1px solid #333', display: 'flex', gap: '8px' }}>
                <input 
                  value={input} 
                  onChange={(e) => setInput(e.target.value)} 
                  onKeyDown={(e) => { if(e.key === 'Enter') { sendToLina(input); setInput(''); } }} 
                  placeholder="toki!"
                  style={{ flex: 1, background: '#1a1a1a', border: '1px solid #333', borderRadius: '8px', padding: '12px', color: 'white', outline: 'none', fontFamily: 'monospace' }} 
                />
                <button 
