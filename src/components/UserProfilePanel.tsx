...
export default function UserProfilePanel({ onClose }: Props) {
  ...
  const tabs: TabID[] = ['IDENTITY', 'PETS', 'PERSONA', 'BELIEFS', 'HEALTH', 'MEDIA', 'DAILY'];

  const handleAddPet = () => {
    if (!newPetName.trim() || !newPetSpecies) return;
    const emoji = PET_SPECIES.find(s => s.label === newPetSpecies)?.emoji ?? '🐾';
    const newPet: Pet = {
    ...
    handleFieldChange('pets', [...(editableProfile.pets || []), newPet]);
    setNewPetName('');
    setNewPetSpecies('');
    setNewPetBreed('');
  };
  ...
  const handleRemovePet = (id: string) => {
    handleFieldChange('pets', (editableProfile.pets || []).filter((p: Pet) => p.id !== id));
  };
            ...
            <div className="tab-content">
              <div style={{ marginBottom: '24px' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginBottom: '20px' }}>
                  {(editableProfile.pets || []).length === 0 ? (
                    <p style={{ color: '#555', fontSize: '0.8rem', fontStyle: 'italic', width: '100%', textAlign: 'center', padding: '24px 0' }}>
                      No pets added yet.
                    </p>
                  ) : (
                    (editableProfile.pets || []).map((pet: Pet) => (
                      <div
                        key={pet.id}
                        style={{
                          display: 'flex',
                      ...
                      >
                        <span style={{ fontSize: '2rem', lineHeight: 1 }}>{pet.emoji ?? '🐾'}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 900, color: 'white', fontSize: '0.9rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {pet.name}
                          </div>
                          <div style={{ fontSize: '0.65rem', color: 'var(--gold)', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                ...
                {isEditing && (
                  <div style={{
                    ...
                    <h4 style={{ fontSize: '0.65rem', color: 'var(--gold)', letterSpacing: '0.12em', margin: '0 0 16px 0', textTransform: 'uppercase' }}>ADD A PET</h4>
                    ...
                    <div style={{ display: 'grid', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '6px' }}>
                          {PET_SPECIES.map(s => (
                            <button
                              key={s.label}
                              type="button"
                              onClick={() => setNewPetSpecies(s.label)}
                              style={{
                                display: 'flex',
                                alignItems: 'center',
                                gap: '5px',
                                ...
                                borderRadius: '20px',
                                border: '1px solid',
                                borderColor: newPetSpecies === s.label ? 'var(--gold)' : 'rgba(255,255,255,0.1)',
                                background: newPetSpecies === s.label ? 'var(--gold)' : 'transparent',
                                color: newPetSpecies === s.label ? 'black' : '#888',
                                fontSize: '0.7rem',
                                fontWeight: newPetSpecies === s.label ? 900 : 500,
                                cursor: 'pointer',
                      ...
                      <div className="field-group" style={{ marginBottom: 0 }}>
                        <label>Breed / Type (optional)</label>
                        <input
                          type="text"
                          value={newPetBreed}
                      ...
                      <button
                        ...
                        disabled={!newPetName.trim() || !newPetSpecies}
                        style={{
                          padding: '12px',
                          background: newPetName.trim() && newPetSpecies ? 'var(--gold)' : 'rgba(255,255,255,0.05)',
                          color: newPetName.trim() && newPetSpecies ? 'black' : '#555',
                          border: 'none',
                          borderRadius: '8px',
                          fontWeight: 900,
                          fontSize: '0.75rem',
                          letterSpacing: '0.08em',
                          cursor: newPetName.trim() && newPetSpecies ? 'pointer' : 'not-allowed',
                          transition: 'all 0.2s'
                        }}
                      >
                        + ADD PET
